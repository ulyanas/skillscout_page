/**
 * Imports official Goldman Sachs skills from the goldmansachs GitHub organization.
 *
 * Goldman Sachs ships skills alongside the library they document (currently
 * gs_quant/skills in goldmansachs/gs-quant), outside the agent runtime
 * directories, so no path exception is needed — this importer just pins the
 * vendor into the directory deterministically instead of waiting for broad
 * discovery to reach it.
 *
 * Run:
 *   node scripts/import_goldman_sachs_skills.mjs
 *
 * Env vars:
...
const SOURCE_ID = "github-goldmansachs-official";
const OWNER_KEY = "goldmansachs";
const OWNER_LOGIN = "goldmansachs";
const OWNER_DISPLAY_NAME = "Goldman Sachs";
const OWNER_WEBSITE = "https://www.goldmansachs.com";
const OWNER_GITHUB_URL = "https://github.com/goldmansachs";
const REPO_FULL_NAMES = ["goldmansachs/gs-quant"];
...
const repos = REPO_FULL_NAMES.map((fullName) => ghApi(`/repos/${fullName}`));
...
for (const repo of repos) {
  ...
  if (!skillPaths.length) {
    throw new Error(`No skills found in ${repo.full_name}`);
  }
...
console.log(`Goldman Sachs: ${owner.skillsCount || 0} skills across ${owner.reposCount || 0} repos`);
...
async function findSkillPaths(repo) {
  ...
  const paths = (tree.tree || [])
    ...
    .map((node) => node.path)
    .filter((filePath) => /(?:^|\/)SKILL\.md$/i.test(filePath))
    .filter((filePath) => {
      const catalogPath = `${normalizeRepoKey(repo.full_name)}/${filePath}`;
...
function upsertOwner(org) {
  ...
  owner.twitter = org.twitter_username ? `@${org.twitter_username}` : owner.twitter;
  owner.lastSeenAt = now;
  owner.firstSeenAt ||= now;

  addUnique(owner.normalizedNames, OWNER_KEY);
  addUnique(owner.normalizedNames, "goldman sachs");
  addUnique(owner.normalizedNames, "goldman");
  addUnique(owner.sourceOwnerKeys, OWNER_LOGIN);
  addUnique(owner.websiteHosts, "goldmansachs.com");
...
function upsertSource() {
  ...
  directory.sources[SOURCE_ID] = {
    sourceId: SOURCE_ID,
    urls: [OWNER_GITHUB_URL, ...REPO_FULL_NAMES.map((name) => `https://github.com/${name}`)],
    fetchedAt: now,
    status: "ok",
    note: "Goldman Sachs first-party skills published alongside their open-source libraries."
  };
}
...
async function fetchRaw(repoFullName, ref, filePath) {
  ...
  const response = await fetch(url, {
    headers: {
      accept: "text/plain",
      "user-agent": "Skillscout Goldman Sachs importer"
    }
  });
  if (!response.ok) {
